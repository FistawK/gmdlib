...
#========CLASSES========
class GDIconKit:
    """A class used to show player's icon kit"""
    def __init__(self, cubeID, shipID, jetpackID, ballID, ufoID, waveID, robotID, spiderID, swingID, color1, color2):
        self.cubeID:int    = cubeID
        self.shipID:int    = shipID
        self.jetpackID:int = jetpackID
        self.ballID:int    = ballID
        self.ufoID:int     = ufoID
        self.waveID:int    = waveID
        self.robotID:int   = robotID
        self.spiderID:int  = spiderID
        self.swingID:int   = swingID
        self.color1:int    = color1
        self.color2:int    = color2
class GDLeader:
    """A class used to indicate a GD player's rank."""
    def __init__(self, rank, accountID, username, stars, demons, usercoins, coins):
        self.rank = rank
        """rank position of the player."""
        self.accountID = accountID
        """accountID of the player."""
        self.username = username
        """username of the player."""
        self.stars = stars
        """stars count of the player."""
        self.demons = demons
        """completed demons count of the player."""
        self.usercoins = usercoins
        """usercoins count of the player."""
        self.coins = coins
        """coins count of the player."""
    def __str__(self):
        return f"<leaderboard player {self.username} with rank {self.rank}>"
class GDLevel:
...
class GDFetchedLevel(GDLevel):
    ...
    Attributes (same as `GDLevel` but without some variables)-
        ...
        version	    The version of the level published
        playerID	The player ID of the level author
        accountID   The account ID of the level author
        username    The username of the level author
        difficultyDenominator	Returns 0 if the level is N/A, returns 10 if a difficulty is assigned. Use `isNA` insted.
        difficultyNumerator     The nominator used for calculating the level difficulty. Divided by the denominator to get the difficulty icon. Nowadays just 0 = unrated, 10 = easy, 20 = normal, 30 = hard, 40 = harder, 50 = insane. Can be also used to determine the demon difficulty as a side-effect of the voting system. Use `difficulty` instead.
        downloads	The amount of times the level has been downloaded
        officialSong	The official song number used by the level, if applicable.
    ...
    """If the level shows a difficulty face of N/A"""
    accountID:int
    """AccountID of the level's author"""
    username:int
    """Username of the level's author"""
    def __init__(self,levelWithHashes:str):
        ...
        self.version:int|None=dic.get('5',None)
        self.playerID:int=int_handled(dic.get('6','0'))
        self.accountID:int=getUserByPlayer(self.playerID).accountID
        self.username:int=getUserByPlayer(self.playerID).username
...
def getUserByPlayer(playerID:str)->GDUser:
    """Gets info about user using player id."""
...
def getUserWithName(name:str)->GDUser:
    ...
    req = requests.post(url, data=data, headers=headers)
    return GDUser(req.text)

def getUserIconKit(accountID)->GDIconKit:
    """Gets icon kit of the player"""
    user = getUserInfo(accountID)
    cube =    user.cubeID
    ship =    user.shipID
    jetpack = user.jetpackID
    ball =    user.ballID
    ufo =     user.ufoID
    wave =    user.waveID
    robot =   user.robotID
    spider =  user.spiderID
    swing =   user.swingID
    col1 =    user.color1
    col2 =    user.color2
    return GDIconKit(
...
def getFromLeaderboard(rank:int)->GDLeader:
    """Gets info about user in leaderboard using rank."""
    rank -= 1
    if rank > 1001:
        return None
    elif rank < 0:
        return None
    r = requests.get('http://boomlings.com/database/accounts/getTop1000.php', headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'})
    soup = BeautifulSoup(r.text, 'html.parser')
    table = soup.find('table')
    if not table:
        return []
    rows = table.find_all('tr')
    headers_row = [h.text for h in rows[0].find_all(['th', 'td'])]
    data = []
    for row in rows[1:]:
        ...
        if cells:
            ...
            for i, cell in enumerate(cells):
                val = cell.text.strip()
                item[headers_row[i]] = int(val) if val.isdigit() else val
            data.append(item)
    return GDLeader(
        rank=rank,
        accountID=data[rank]['AccountID'],
        username=data[rank]['Username'],
        stars=data[rank]['Stars'],
        demons=data[rank]['Demons'],
        usercoins=data[rank]['UserCoins'],
        coins=data[rank]['Coins']
    )
